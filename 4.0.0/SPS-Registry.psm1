...
Function Get-SPSRegistry {
    ...
    .PARAMETER Strict
    Set the strict mode for the function. any unknown line will throw an error.
    .EXAMPLE
    Get-SPSRegistryContent -File 'C:\Temp\test.reg'
    Read the file 'C:\Temp\test.reg' and convert it to a Registry object.
    .EXAMPLE
    Get-SPSRegistry -File (Get-ChildItem -Path 'C:\Temp' -Filter '*.reg')
    ...
    Param(
        ...
        [Parameter(
            Position = 2,
            Mandatory = $False,
            HelpMessage = 'Set the strict mode for the function. if set the function throw error when reaching unknow line.',
            ParameterSetName = 'ByFile'
        )]
        [Switch] ${Strict},
        [Parameter(
    ...
    PROCESS {
        ...
        if ($PSCmdlet.ParameterSetName -eq 'ByItem') {
            ...
            ForEach($Item in $RegPath) {
                ...
                if ($Item.StartsWith('HKEY_')) {
                    Switch ($Item) {
                        {$_ -like 'HKEY_CLASSES_ROOT\*'} {$Item = $Item.Replace('HKEY_CLASSES_ROOT','HKCR:');BREAK}
                        {$_ -like 'HKEY_CURRENT_USER\*'} {$Item = $Item.Replace('HKEY_CURRENT_USER','HKCU:');BREAK}
                        {$_ -like 'HKEY_LOCAL_MACHINE\*'} {$Item = $Item.Replace('HKEY_LOCAL_MACHINE','HKLM:');BREAK}
                        {$_ -like 'HKEY_USERS\*'} {$Item = $Item.Replace('HKEY_USERS','HKU:');BREAK}
                        default {Throw "Unknown Hive: $($Item)"}
                    }
                }
            }
        }Elseif ($PSCmdlet.ParameterSetName -eq 'ByFile') {
            # Handle the reg file
            ForEach($SingleFile in $RegFile){
                try {
...
Function Merge-SPSRegistry {
    ...
    .EXAMPLE
    $Reg1 = Get-SPSRegistry -File 'C:\Temp\test1.reg'
    $Reg2 = Get-SPSRegistry -File 'C:\Temp\test2.reg'
    Merge-SPSRegistry -InputObject $Reg1,$Reg2
    Merge the two registry objects into a single registry object.
    .EXAMPLE
    Merge-SPSRegistry -Path 'C:\Temp'
    ...
    BEGIN {
        ...
        if ($PSCmdlet.ParameterSetName -eq 'byPath') {
            $InputObject = Get-ChildItem -Path $Path -File | Get-SPSRegistry -Strict:$Strict
        }
...
ForEach ($Type in $ExportableTypes) {
    if ($Type.FullName -in $ExistingTypeAccelerators.Keys) {
        ...
        $RegisterableTypes += $Type
    }
}
# Add type accelerators for every exportable type.
ForEach ($Type in $RegisterableTypes) {
